...
const upvoteComment = (score_selector) => {
    ...
    plusSign.forEach((sign, index) => {
        //For each plus sign img
        ...
        sign.addEventListener("click", function upvote(ev) {
            //Upvote func
            //Apply filter to image
            sign.style =
                "filter: brightness(0) saturate(100%) invert(42%) sepia(74%) saturate(556%) hue-rotate(200deg) brightness(73%) contrast(93%);";
            //Remove minusSign img style
            minusSign[index].style = " ";
            if (wasUpvoted === false) {
                // if wasUpvoted flag === false
            ...
            sign.addEventListener("click", function cancelUpvote() {
                //Cancel upvote func
...
const downvoteComment = (score_selector) => {
    ...
    minusSign.forEach((sign, index) => {
        //For each minus sign img
...
const deleteStylingOldComments = (delete_selector) => {
    delete_selector.forEach((container, index) => {
        //For each container that has both of Edit and Delete containers
        ...
        container.addEventListener("mouseenter", () => {
            //On mouse enter
            /*Apply filters*/
            trashBin[index].style =
                "filter: brightness(0) saturate(100%) invert(85%) sepia(88%) saturate(7056%) hue-rotate(289deg) brightness(104%) contrast(104%);"; //add filter
    ...
    edit_selector.forEach((container, index) => {
        ...
        container.addEventListener("mouseenter", () => {
            pencilImage[index].style =
                "filter: brightness(0) saturate(100%) invert(79%) sepia(29%) saturate(532%) hue-rotate(201deg) brightness(100%) contrast(88%);";
        ...
        container.addEventListener("click", (evt) => {
            ...
            let comment_text =
                evt.currentTarget.parentNode.parentNode.children[1].textContent; //Nearest comment text
            ...
            updateButton.addEventListener("click", (ev) => {
                ...
                let span_created_at =
                    ev.currentTarget.parentNode.children[0].children[3]; //Created at
                ...
                if (
                    span_text != undefined &&
                    text_area.value.includes(span_text.textContent)
                ) {
                    let textWithoutUser = text_area.value.replace(
                    ...
                    if (!user_data.textContent.includes("Edited")) {
                        ...
                        edited_span.style =
                            "color: var(--neutral-grayish-blue); text-decoration: underline;";
                        edited_span.innerText = "Edited";
                        span_created_at.insertAdjacentElement("afterend", edited_span);
                    }
                } else if (span_text == undefined) {
                    //If the span selector ev.target is different than the one defined, do the same thing
...
const editNewFunctionality = (edit_new_single, comment_new, edit) => {
    edit_new_single.addEventListener("click", (evt) => {
        //The specific new Edit container to attach the event listener

        /*The specific comment selector*/
        const comment_html = document.querySelector(
            `${comment_new}` +
            CSS.escape(
                evt.currentTarget.parentNode.children[1].id.replace(`${edit}`, "")
            )
        );
        comment_html.style = "display: none"; //Hide the comment

        let comment_text =
            comment_html.textContent; /*edit_btn.closest('div.reply').querySelector('p.content').textContent;*/
        ...
        updateButton.addEventListener("click", (ev) => {
            ...
            const span_created_at =
                ev.currentTarget.parentNode.children[0].lastElementChild; //Created at
            ...
            ev.currentTarget.parentNode.removeChild(
                ev.currentTarget.parentNode.children[4]
            ); //Remove update button from card; for some reason it does not work properly with updatebtn variable
...
const Modal = (delete_selector, main) => {
    ...
    delete_selector.forEach((elem) => {
        elem.addEventListener("click", (event) => {
            //For each delete container elem, onclick
      ...
      `;
            ...
            if (document.querySelectorAll(".dialog")) {
                //If there are elements with dialog class

                const cancel = document.querySelectorAll(".cancel"); //Select all cancel btns
                const deleteCommentModal = document.querySelectorAll(
                    ".delete-modal-button"
                ); //Select all delete comment btn from the modal
                cancel.forEach((button) => {
                    //For each canel btn
                    button.addEventListener("click", (ev) => {
                        //On click, remove the modal
                ...
                deleteCommentModal.forEach((button) => {
                    //For each delete comment btn from modal
                    button.addEventListener("click", (delete_event) => {
                        //On click
                        const background = document.querySelector(".background");
                        const dialog = document.querySelector(".dialog");
...
const createNewComment = (textarea, insertcomment, comments_d) => {
    //Create new comment function
    if (textarea.value != "") {
        //Allow the user to enter comment as long as the textarea is not empty
...
<!--User data-->
<div class="user-data d-flex align-items-center gap-3 col-lg-8">
  <img class="profile-picture" src="${comments_d.currentUser.image.webp
      }" alt="profile picture">
  ...
  <!--Comment content-->
  <p id="comment-new-${newCommentCounter}" class="comment comment-${dataIndexId_number + 1}">${textarea.value}
  </p>
  <!--Comment score-->
  <div class="comment-score d-flex align-items-center justify-content-between">
  <img class="plus-sign" src="./images/icon-plus.svg" alt="plus">
  ...
  <a class="edit-button" href="#!">Edit</a>
        ...
        const delete_new_single = document.querySelector(
            "#delete-" + CSS.escape(newCommentCounter)
        ); //Select the newest comment's delete container
        const edit_new_single = document.querySelector(
            "#edit-" + CSS.escape(newCommentCounter)
        ); //Select the newest comment's edit container
...
const insertReply = (btn) => {

        //Insert reply functionality
        btn.addEventListener("click", (ev) => {
                    //Add button click event
                    const parent_comment_selector = $(ev.currentTarget.parentNode.parentNode.parentNode).prevAll('.comment-div').first(); //Get the closest comment (upwards)
                    const parent_comment_selector_id = parent_comment_selector.attr('data-index'); //Get the data-index of the comment

                    const p = document.createElement("p");
                    p.classList.add("content");
                    const textar =
                        ev.currentTarget.parentNode.parentNode.parentNode.children[0].value; //Text area selector
                    if (textar != undefined) {
                        const replying_to =
                            ev.currentTarget.parentNode.parentNode.parentNode.children[0].textContent.split(
                                ","
                            ); //Replying to text, to be inserted later into span
                        ...
                        the_div.dataset.parent = parent_comment_selector_id;
      ...
      <!--Reply content-->
      <p id="reply-new-${newReplyCounter}" class="content comment-${max}">
      <span class="replying-to">${replying_to[0]}</span>${textar.replace(
        `${replying_to[0]}`,
        ""
      )}
      </p>
      ...
      const delete_new_single = document.querySelector(
        "#delete-reply-" + CSS.escape(newReplyCounter)
      ); //Select the newly created Delete button
      const edit_new_single = document.querySelector(
        "#edit-reply-" + CSS.escape(newReplyCounter)
    ...
    } else if (textar == undefined) {
      // If ev.currentTarget.parentNode.parentNode.parentNode.children[0].value does not exist (eg. different elements in the card);
      const textarrMobile =
        ev.currentTarget.parentNode.parentNode.children[0].value;
      const replying_to =
        ev.currentTarget.parentNode.parentNode.children[0].textContent.split(
          ","
        );
...
const replyToComment = (reply_button_container, data) => {
  reply_button_container.forEach((btn) => {
    //For each btn inside reply container (eg. image and text)

    //On click
    btn.addEventListener("click", (ev) => {
      const parent_comment_selector = $(ev.currentTarget.parentNode).prevAll('.comment-div').first(); //Get the closest comment (upwards)
      const parent_comment_selector_id = parent_comment_selector.attr('data-index'); //Get the data-index of the comment
      const comment_select = ev.currentTarget.parentNode; //Comment selector

      const comment_select_username =
        ev.currentTarget.parentNode.children[0].children[1].textContent; //Username selector
      ...
      const replyCard_index_no =
        ev.currentTarget.parentNode.getAttribute("data-index"); //Get the parent's data-index attribute

      const responsiveReply = (replyCard, comment_select) => {
        if (contains_reply_class == true) {
          //Checks if the comment has the class "reply"
          replyCard.classList.add("is-reply"); //Apply reply styling - aka shorter width than the comment
        }

        if (desktopMediaQuery.matches) {
          //Reply card gets this HTML structure on desktop
        ...
        <button type="button" id="reply-new-${replyCard_index_no}" class="btn">REPLY</button>
          ...
          // Reply card gets this HTML structure on mobile
    ...
    `;
        ...
        const reply_id_select = document.querySelector(
          "#reply-new-" + CSS.escape(replyCard_index_no)
...
const getCurrentUser = (comments_data) => {
  ...
  data_indexes.forEach((elem, index) => {
    if (
      elem.innerHTML.includes(
        `<span class=\"username\">${comments_data.currentUser.username}</span>`
      )
    ) {
...
//////////////////////////////////Main App///////////////////////////////////////////////////////
//Fetch data from server/LocalStorage
const fetchData = async () => {

  if (window.localStorage.getItem('data')) { //If there is such item in localStorage, use it in createApp();
    ...
    //else use the data provided from the server
  } else {
...
const createApp = async (fetchedJson) => {
  ...
  addEventListeners(); //Add main event listeners


  /*Save state in LocalStorage just before the user leaves the page*/
  window.addEventListener("beforeunload", () => {
    const comment_div = document.querySelectorAll(".comment-div"); //Select all the comments
    const data_index = document.querySelectorAll(".comment-div[data-index]"); //Get the comments's data-index
    const user = document.querySelectorAll(".comment-div .user-data"); //Get the comment's user info

    const p_comment_div = document.querySelectorAll(".comment-div p"); //Get the comment's content
    const createdAt = document.querySelectorAll(".comment-div span.created-at"); //Get the created at info
    const score = document.querySelectorAll(".comment-div .score-number"); //Get the score number 





    let commentsObject = []; //Create an empty array

    let currentUserObject = { //Current user object
      "currentUser": {
        "image": {
          ...
          commentsObject[id].replies.push(
            {
              ...
              "user": {
                "image": {
                  "webp": `./images/avatars/image-${image_url}.webp`
                },
                "username": username
              }
            }
    ...
    Object.assign(currentUserObject, { "comments": commentsObject }); //Append the comments object to the current user object
    window.localStorage.setItem('data', JSON.stringify(currentUserObject)); //Store in LocalStorage
